...
<?php
	...
	$cek_smnr = mysqli_num_rows(mysqli_query($connect, "SELECT nm_seminar FROM seminar
										   WHERE nm_seminar='$nm_seminar'"));
	...
	if (trim($biaya)=="" or ! is_numeric(trim($biaya))) {
		...
		echo "<script>alert('Data seminar sudah tersimpan.');</script>";
