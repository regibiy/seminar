...
<?php
	...
	$cek_edu = mysqli_num_rows(mysqli_query($connect, "SELECT pendidikan FROM pendidikan
										   WHERE pendidikan = '$pendidikan'"));

	if($cek_edu > 0){
		echo" <script>alert('Nama pendidikan sudah digunakan. Silahkan gunakan nama pendidikan yang lain.');</script>";
		echo "<meta http-equiv='refresh' content='0;url=?page=adEdu'>";
	}
		...
		echo "<script>alert('Data pendidikan sudah tersimpan.');</script>";
